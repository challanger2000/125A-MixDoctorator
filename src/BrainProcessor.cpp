...
tresult PLUGIN_API Processor::setBusArrangements(
    SpeakerArrangement* in,int32 ni,SpeakerArrangement* out,int32 no){
...
tresult PLUGIN_API Processor::setProcessing(TBool state){
    if(state){
        for(auto& p:pairStates_){
            p.score=0.0; p.observedSeconds=0.0; p.dominance=0.0; p.confidence=0.0;
            p.dominantBand=0; p.previousBand=0;
...
void Processor::publishParam(ProcessData& data,Steinberg::Vst::ParamID id,double v,int slot){
    ...
    if(!q) return;
    int32 point=0;
    if(q->addPoint(0,v,point)==kResultTrue) last_[slot]=v;
}

void Processor::updatePair(
    const IPC::Snapshot& a,const IPC::Snapshot& b,PairState& state,int32 numSamples) noexcept{

    const double dt=std::clamp(
        static_cast<double>(std::max<int32>(1,numSamples))/std::max(8000.0,sampleRate_),
        0.0001,0.25);

    const bool active=a.connected && b.connected && a.rmsDb>-55.0 && b.rmsDb>-55.0;

    double target=0.0;
    double commonBands[IPC::kBandCount]{0,0,0,0,0};
    ...
    if(active){
        ...
        for(int i=0;i<IPC::kBandCount;++i){
            commonBands[i]=std::min(std::max(0.0,a.bands[i]),std::max(0.0,b.bands[i]));
            rawOverlap+=commonBands[i];
        }
        rawOverlap=std::clamp(rawOverlap,0.0,1.0);

        const double jointActivity=std::sqrt(
            std::clamp(a.activity,0.0,1.0)*std::clamp(b.activity,0.0,1.0));
    ...
    const double alpha=1.0-std::exp(-dt/((target>state.score)?0.65:3.0));
    ...
    if(active){
        ...
        for(double& value:state.bands) value+=releaseAlpha*(0.0-value);
    }
    ...
    if(active){
        const double aAmp=std::pow(10.0,a.rmsDb/20.0)*std::sqrt(std::max(0.0,a.bands[best]));
        const double bAmp=std::pow(10.0,b.rmsDb/20.0)*std::sqrt(std::max(0.0,b.bands[best]));
        const double aDb=20.0*std::log10(std::max(aAmp,1.0e-9));
        const double bDb=20.0*std::log10(std::max(bAmp,1.0e-9));
        const double targetDominance=std::clamp((aDb-bDb)/12.0,-1.0,1.0);
        const double domAlpha=1.0-std::exp(-dt/1.5);
        state.dominance+=domAlpha*(targetDominance-state.dominance);
    }

    const double timeConfidence=std::clamp(state.observedSeconds/8.0,0.0,1.0);
    const double bandStability=sameBand ? 1.0 : 0.45;
    const double scoreConfidence=std::clamp((state.score-0.10)/0.35,0.0,1.0);
    const double targetConfidence=timeConfidence*(0.55*scoreConfidence+0.45*bandStability);
    const double confAlpha=1.0-std::exp(-dt/2.0);
    state.confidence+=confAlpha*(targetConfidence-state.confidence);
}
...
double Processor::dominanceParam(double d) noexcept{
    if(d>0.20) return 1.0;
    if(d<-0.20) return 0.0;
    return 0.5;
}

int Processor::adviceFor(int pairIndex,int bandIndex,double dominance) noexcept{
    const int side=dominance>0.20 ? 1 : dominance<-0.20 ? -1 : 0;
...
int Processor::chooseTopPair() noexcept{
    auto eligible=[this](int i){
        return pairStates_[i].observedSeconds>=2.0 &&
               ...
               pairStates_[i].confidence>=0.15;
    };

    if(heldTopPair_>=0 && heldTopPair_<3 && eligible(heldTopPair_)){
        int challenger=heldTopPair_;
        double challengerRank=
            pairStates_[heldTopPair_].score*(0.65+0.35*pairStates_[heldTopPair_].confidence);

        for(int i=0;i<3;++i){
            if(!eligible(i)) continue;
            const double rank=pairStates_[i].score*(0.65+0.35*pairStates_[i].confidence);
            if(rank>challengerRank+0.05){
                challenger=i;
                challengerRank=rank;
            }
        }
...
tresult PLUGIN_API Processor::process(ProcessData& data){
    ...
    const double dominanceValue=(best<0)?0.5:dominanceParam(pairStates_[best].dominance);
    const double confidenceValue=(best<0)?0.0:pairStates_[best].confidence;

    const int advice=(best<0)?0:
        adviceFor(best,pairStates_[best].dominantBand,pairStates_[best].dominance);
